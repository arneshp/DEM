...
// Since this is the first time these properties such as Sdim, Swratio and iradi are defined. 
// I have removed the if statement to check if they are already defined. There is a problem with mpirun 
// if the if statement is included. so, everytime this fix is called these parameter will be reinitialised. 
...
void FixHeatGran_modified::init()
{
  ...
  updatePtrs();
   ...
   bigint natoms = atom->natoms; // natoms is the total number of atoms in the system. 
...
void FixHeatGran_modified::post_force(int vflag) 
{
  updatePtrs();
 double *radius = atom->radius;
 double tdelay_ref;
