...
namespace ContactModels
{
  ...
  public:
	...
	double Yi,Yj,Yeff;
      ...
      fix_Swratio = static_cast<FixPropertyAtom*>(modify->find_fix_property("Swratio","property/atom","scalar",0,0,"Swratio"));
      Swratio = fix_Swratio->vector_atom; 	
	
      fix_iradi = static_cast<FixPropertyAtom*>(modify->find_fix_property("iradi","property/atom","scalar",0,0,"iradi"));
      iradi = fix_iradi->vector_atom; 

	double vfwateri,vfwaterj; 
	
	vfwateri = (pow(radi/iradi[i],3)-1; 
	vfwaterj = (pow(radj/iradi[j],3)-1;
	
	
	Yi = Yh[itype]*(1-vfwateri)+Yl[itype]*vfwateri;
	Yj = Yh[itype]*(1-vfwaterj)+Yl[jtype]*vfwateri;
  ...
  protected:
    ...
    double *Sdim; // degree of swelling. 
    double Yeff;
    double Geff;
    double *iradi; // initial radius. 
    double *Swratio; //swelling ratio. 
