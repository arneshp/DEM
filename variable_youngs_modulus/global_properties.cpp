...
namespace MODEL_PARAMS
{
 ...
 /* -------------------------------------------------------------- */
   VectorProperty * createYoungsModuluslow(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    LAMMPS * lmp = registry.getLAMMPS();
    const int max_type = registry.max_type();

    VectorProperty * vec = new VectorProperty(max_type+1);
    FixPropertyGlobal * Y = registry.getGlobalProperty("youngsModuluslow","property/global","peratomtype",max_type,0,caller);
 ...
 VectorProperty * createYoungsModulushigh(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    LAMMPS * lmp = registry.getLAMMPS();
    const int max_type = registry.max_type();

    VectorProperty * vec = new VectorProperty(max_type+1);
    FixPropertyGlobal * Y = registry.getGlobalProperty("youngsModulushigh","property/global","peratomtype",max_type,0,caller);
