...
TANGENTIAL_MODEL(TANGENTIAL_HISTORY_SWELLING,history_swelling,5)
#else
#ifndef TANGENTIAL_MODEL_HISTORY_SWELLING_H_
#define TANGENTIAL_MODEL_HISTORY_SWELLING_H_
...
{
  template<>
  class TangentialModel<TANGENTIAL_HISTORY_SWELLING> : public TangentialModelBase
  {
    double ** coeffFrict;
    int history_offset;
  ...
  public:
    ...
    inline void surfacesClose(SurfacesCloseData & scdata, ForceData&, ForceData&)
    {
    	...
    	{
        const double enx = scdata.delta[0]/r;
        const double eny = scdata.delta[1]/r;
        const double enz = scdata.delta[2]/r;
    
    	  double * const shear = &sidata.contact_history[history_offset];
    
    	if (update_history) {
          const double dt = update->dt;
          shear[0] += scdata.vtr1 * dt;
          shear[1] += scdata.vtr2 * dt;
          shear[2] += scdata.vtr3 * dt;
    	...
    	const double shrmag = sqrt(shear[0]*shear[0] + shear[1]*shear[1] + shear[2]*shear[2]);
        const double kt = scdata.kt;
        const double xmu = coeffFrict[scdata.itype][scdata.jtype];
	...
	Lcj = Lcbarj*pow(M_PI,3)*(Yj/6000)*pow(Cvj,3)*pow(((roughnessRatio*radj)*(2*(1-nu[jtype]*nu[jtype])/1000)),2);
        ...
        xmui = 0.27*coth(0.27*(pow(scdata.Fn[0]*scdata.Fn[0]+scdata.Fn[1]*scdata.Fn[1]+scdata.Fn[2]*scdataFn[2],0.5))/Lci);
        xmuj = 0.27*coth(0.27*(pow(scdata.Fn[0]*scdata.Fn[0]+scdata.Fn[1]*scdata.Fn[1]+scdata.Fn[2]*scdataFn[2],0.5))/Lcj);
    
     // rescale frictional displacements and forces if needed
        const double Ft_shear = kt * shrmag; // sqrt(Ft1 * Ft1 + Ft2 * Ft2 + Ft3 * Ft3);
        const double Ft_friction = xmu * fabs(scdata.Fn);
	...
	double torque_i[3];
	    ...
	    torque_i[0] = -scdata.cri * tor1;
            torque_i[1] = -scdata.cri * tor2;
            torque_i[2] = -scdata.cri * tor3;
	 	torque_j[0] = -scdata.crj * tor1;
                torque_j[1] = -scdata.crj * tor2;
                torque_j[2] = -scdata.crj * tor3;
	


	 // return resulting forces, only implemented for particle pairs.
